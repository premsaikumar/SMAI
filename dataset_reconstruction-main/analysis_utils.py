...
from Main import setup_args, ec
...
def sweep_get_data_model(sweep, verbose=True, run_train_test=False, put_in_sweep=True):
    ...
    if run_train_test:
        ...
        trn_error, trn_loss, trn_vals = ec(args, train_loader, model, epoch=-1, device=args.device, opt=None)
        if verbose: print('Train Error:', trn_error, trn_loss)
        tst_error, tst_loss, tst_vals = ec(args, test_loader, model, epoch=-1, device=args.device, opt=None)
